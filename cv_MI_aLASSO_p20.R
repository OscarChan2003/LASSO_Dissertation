################################################################################
#                                                                              # 
# Here we develop the MI.alasso.cv function based on the code of MI.lasso      #
...
# MI-aLASSO mentioned in our paper. The MI.alasso.cv computes the weight term  #
# based on the procedure described by Du et al. (2022).                        #
#                                                                              # 
# References:                                                                  # 
#                                                                              # 
...
#### MI.lasso function
MI.alasso.cv = function(mydata, D, weight = weight, lamvec = lamvec, k = 5, use.1se = TRUE, maxiter = 200, eps = 1e-6, seed) {
  ...
  for (fold in 1:k) {
    ...
    for (i in 1:length(lamvec)) {
      ...
      while (iter <= maxiter & dif >= eps) {
        ...
        for (d in 1:D) {
          xtx = t(x[[d]]) %*% x[[d]]
          diag(xtx) = diag(xtx) + lambda * weight / c
          xty = t(x[[d]]) %*% y[[d]]
...
for (miss_prop in missing_props) {
  for (snr in SNR_list) {
    for (seed in st.seed:(st.seed + replicate - 1)) {
      ...
      initial_fit <- MI.alasso.cv(mydata = newdata, D = D, weight = rep(1, p), lamvec = lamvec, use.1se = TRUE, seed = seed)
      
      weight <- rep(1, p)
      sqrt_bj <- sqrt(rowSums(initial_fit$coefficients[-1, ]^2))
      v <- log(p * D) / log(n * D)
      gamma <- ceiling((2 * v) / (1 - v)) + 1
      weight <- (sqrt_bj + (1 / (n * D)))^(-gamma)
      
      model_fit <- MI.alasso.cv(mydata = newdata, D = D, weight = weight, lamvec = lamvec, use.1se = TRUE, seed = seed)
