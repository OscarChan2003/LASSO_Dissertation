...
for (miss_prop in missing_props) {
  for (snr in SNR_list) {
    for (seed in st.seed:(st.seed + replicate - 1)) {
      ...
      cv_fit1 <- cv.galasso(x_list, y_list, adWeight = adWeight1, pf = pf) 
      best_lambda1 <- cv_fit1$lambda.1se
      glasso_coef_list <- coef(cv_fit1, lambda = best_lambda1)
      model_fit1 <- process_galasso_output(glasso_coef_list, p, D)
      
      weight <- rep(1, p)
      sqrt_bj <- sqrt(rowSums(model_fit1$coefficients[-1, ]^2))
      v <- log(p * D) / log(n * D)
      gamma <- ceiling((2 * v) / (1 - v)) + 1
      weight <- (sqrt_bj + (1 / (n * D)))^(-gamma)
      
      cv_fit <- cv.galasso(x_list, y_list, adWeight = weight, pf = pf) 
