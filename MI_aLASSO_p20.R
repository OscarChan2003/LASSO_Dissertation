...
MI.alasso = function(mydata=mydata, D, weight, lambda, maxiter=200, eps=1e-6) {
...
for (miss_prop in missing_props) {
  for (snr in SNR_list) {
    for (seed in st.seed:(st.seed + replicate - 1)) {
      ...
      initial_fit <- MI.lasso(mydata = newdata, D = D, lambda = lamvec[opt])
      ...
      bicvec <- sapply(lamvec, function(lam) {
        fit <- MI.alasso(mydata = newdata, weight = weight, D = D, lambda = lam)
        fit$bic
      })
      opt <- which.min(bicvec)
      
      model_fit <- MI.alasso(mydata = newdata, weight = weight, D = D, lambda = lamvec[opt])
